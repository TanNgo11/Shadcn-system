...
import {Button, Card, Modal, Form, Input, InputNumber, Select, Table, Space, Popconfirm} from 'antd';
import {PlusOutlined, EditOutlined, DeleteOutlined, ArrowLeftOutlined} from '@ant-design/icons';
import {RoomResponse, RoomUpdateRequest, RoomType, RoomStatus} from '@/queries/Building/types';
import {useDeleteRoom, useGetBuildingById, useUpdateRoom} from '@/queries/Building';
import {ActionType} from '@ant-design/pro-components';
import {useGetRoomsByBuilding} from '@/queries/Building';
...
export default function ViewRoomsBuilding() {
	...
	const handleEditRoom = async (_roomId: string, roomUpdateRequest: RoomUpdateRequest) => {
		try {
			setLoading(true);
			updateRoom({
				request: roomUpdateRequest,
				roomId: roomUpdateRequest.code
			});
			toast.success({
			...
			toast.error({
				message: 'Update Room',
				description: 'The room could not be updated.',
			});
		} finally {
			setLoading(false);
		}
	...
	return (
		<div>
			...
			<Modal
				title={modalTitle}
				open={isModalVisible}
				onOk={() => handleEditRoom(editingRoom?.code || '', form.getFieldsValue())}
